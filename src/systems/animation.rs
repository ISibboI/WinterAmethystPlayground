...
impl<'s> System<'s> for AnimationSystem {
    type SystemData = (WriteStorage<'s, Animated>, ReadStorage<'s, WorldCollisionAffected>, WriteStorage<'s, SpriteRender>, Read<'s, Time>, Read<'s, InputHandler<String, String>>);

    fn run(&mut self, (mut animateds, world_collision_affecteds, mut sprite_renders, time, input_handler): <Self as System<'s>>::SystemData) {
        for (mut animated, world_collision_affected, mut sprite_render) in (&mut animateds, &world_collision_affecteds, &mut sprite_renders).join() {
